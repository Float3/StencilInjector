...
namespace _3.Editor
{
    public class ParsedShader
    {
        ...
        private readonly ZipArchive zipArchive = openBuiltinShaderZip();

        public ParsedShader(Shader shader)
        {
            this.shader = shader;
            filePath = getPath(shader);
            Parse();
        }
                    ...
                    switch (state)
                    {
                        ...
                        case ParseState.PassCG:
                            ...
                            Debug.Log ("Ending cg:" + cgType + " lines " + beginCGLineNum + "-" + lineNum);
                                ...
                                {
                                    int whichBlock = 0;
                                    for (int i = 0; i < beginCGLineNum; i++) // if (i == cgBlocks[whichBlock].beginLineNum) {
                                        //     buf += shaderLines[i].Substring(cgBlocks[whichBlock].beginSkip) + "\n";
                                        // } else
                                        if (whichBlock >= cgBlocks.Count)
                                            buf += "\n";
                                        else if (i >= cgBlocks[whichBlock].beginLineNum && i < cgBlocks[whichBlock].endLineNum) buf += shaderLines[i] + "\n";
        ...
        public class CGBlock : Block
        {
            ...
            public CGBlock(ParsedShader shader, Type type, int beginLine, int endLine, string cgProgramSource) : base(shader, type, beginLine, 0, endLine, 0)
            {
                ...
                else
                {
                    ...
                    {
                        if (lin.IndexOf("// Surface shader code generated based on:", StringComparison.CurrentCulture) != -1) originalSurfaceShader = true;
            ...
            private class CGProgramOutputCollector : Preproc.OutputInterface
            {
                private readonly StringBuilder outputCode = new StringBuilder();
                private readonly ParsedShader parsedShader;
                private bool wasNewline;
